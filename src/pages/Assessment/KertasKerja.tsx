                            ...
                            <div className="mt-3 space-y-2">
                              {factor.subFactors.map(sf => (
                                <div key={sf.id} className="text-xs bg-gray-50 p-2.5 rounded border border-gray-200 flex items-start gap-2 relative overflow-hidden">
                                  <div className={`absolute left-0 top-0 bottom-0 w-1 ${(sf.subFactorScore || 0) >= 1.00 ? 'bg-green-500' :
                                      (sf.subFactorScore || 0) >= 0.75 ? 'bg-blue-400' :
                                        (sf.subFactorScore || 0) >= 0.50 ? 'bg-amber-400' :
                                          (sf.subFactorScore || 0) > 0 ? 'bg-orange-400' : 'bg-gray-300'
                                    }`}></div>
                                  <span className="font-bold text-gray-500 pl-2">{sf.id}.</span>
                                  <div className="flex justify-between items-start w-full">
                                    <div>
                                      <span className={`font-semibold block ${(sf.subFactorScore || 0) > 0 ? 'text-blue-700' : 'text-gray-700'
                                        }`}>{sf.name}</span>
                                      {sf.description && <span className="text-gray-500 italic block mt-0.5">{sf.description}</span>}
                                    </div>
                                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ml-2 shrink-0 ${(sf.subFactorScore || 0) >= 1.00 ? 'bg-green-100 text-green-700' :
                                        (sf.subFactorScore || 0) >= 0.75 ? 'bg-blue-100 text-blue-700' :
                                          (sf.subFactorScore || 0) >= 0.50 ? 'bg-amber-100 text-amber-700' :
                                            (sf.subFactorScore || 0) > 0 ? 'bg-orange-100 text-orange-700' :
                                              'bg-gray-100 text-gray-500'
                                      }`}>
                                      {Number(sf.subFactorScore || 0).toFixed(2)}
                                    </span>
                                  </div>
                                </div>
                              ))}
                    ...
                    {modalConfig.data.subFactors && modalConfig.data.subFactors.length > 0 ? (
                      <div className="space-y-3 bg-gray-50/50 p-5 rounded-xl border border-gray-200">
                        ...
                        {modalConfig.data.subFactors.map((sf: any, sfIndex: number) => (
                          <div key={sf.id} className={`flex items-start gap-4 p-3 bg-white rounded-xl border border-gray-200 transition-all shadow-sm ${isReadOnly ? 'opacity-70 cursor-not-allowed' : 'hover:bg-blue-50/30 hover:border-blue-300'}`}>
                            <div>
                              <select
                                disabled={isReadOnly}
                                className="text-xs font-bold border border-gray-300 rounded px-2 py-1.5 bg-white outline-none focus:ring-1 focus:ring-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed mt-1"
                                value={sf.subFactorScore || 0}
                                onChange={(e) => {
                                  const updatedSubFactors = [...modalConfig.data.subFactors];
                                  updatedSubFactors[sfIndex] = {
                                    ...updatedSubFactors[sfIndex],
                                    subFactorScore: Number(e.target.value)
                                  };
                                  const avgScore = updatedSubFactors.reduce(
                                    (sum, s) => sum + (Number(s.subFactorScore) || 0), 0
                                  ) / (updatedSubFactors.length || 1);
                                  setModalConfig({ ...modalConfig, data: { ...modalConfig.data, subFactors: updatedSubFactors, factorScore: avgScore } });
                                }}
                              >
                                <option value={0.00}>0.00 — Tidak ada / 0%</option>
                                <option value={0.25}>0.25 — Sebagian kecil (&gt;0–50%)</option>
                                <option value={0.50}>0.50 — Sebagian (50–75%)</option>
                                <option value={0.75}>0.75 — Sebagian besar (75–85%)</option>
                                <option value={1.00}>1.00 — Seluruhnya (&gt;85%)</option>
                              </select>
                            </div>
                            <div className="flex-1">
                              <span className={`font-bold text-sm block ${sf.subFactorScore > 0 ? 'text-blue-700' : 'text-gray-700'}`}>{sf.name}</span>
