...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`bg-white border-r border-indigo-100/50 h-screen fixed left-0 top-0 flex flex-col transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] z-50 overflow-hidden shadow-[4px_0_24px_rgba(0,0,0,0.02)] ${isMobile ? (isOpen ? 'w-64 translate-x-0' : 'w-64 -translate-x-full') : (isOpen ? 'w-64' : 'w-20')
          }`}
      >
        {/* HEADER BRANDING */}
        <div className="h-[68px] px-6 flex items-center shrink-0 border-b border-slate-50 relative group">
          <div
          ...
          >
            <div className={`flex flex-col justify-center overflow-hidden ${(isOpen || isMobile) ? 'w-auto' : 'w-0'}`}>
              <h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-baseline">
                <span className="relative inline-flex flex-col items-start">
                  ...
                  <span className="absolute -bottom-1 left-0 w-5 h-[3px] bg-violet-500 rounded-full"></span>
                </span>
              </h2>
              <p className="text-[8.5px] font-bold text-slate-400 mt-2 uppercase tracking-widest whitespace-nowrap">
                ...
                {group.title && (
                  <h3
                    className={`text-[10px] font-black text-slate-400 tracking-widest uppercase transition-all duration-300 ${
                      isOpen || isMobile ? 'opacity-100 flex px-3' : 'opacity-0 hidden px-0'
                    }`}
                  >
                    {group.title}
                  </h3>
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center py-3 rounded-2xl transition-all duration-200 group ${
                          isActive
                            ? 'bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100/50'
                            : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900 transparent border border-transparent'
                        } ${(!isOpen && !isMobile) ? 'justify-center px-0' : 'px-3'}`}
                      >
                        ...
                        <span
                          className={`font-bold text-[13px] whitespace-nowrap transition-all duration-300 ${
                            (isOpen || isMobile) ? 'opacity-100 translate-x-0 block ml-3' : 'opacity-0 -translate-x-4 hidden ml-0'
                          }`}
                        >
                          {item.label}
                        </span>
        ...
        {/* LOGOUT */}
        <div className="p-3 border-t border-slate-50 shrink-0 mt-auto bg-slate-50/50">
          <button
            onClick={signOut}
            title={(!isOpen && !isMobile) ? 'Keluar Sistem' : undefined}
            className={`w-full flex items-center justify-center py-3 rounded-2xl text-slate-500 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-100 transition-all duration-200 border border-transparent ${
              (!isOpen && !isMobile) ? 'px-0' : 'px-3'
            }`}
          >
            <LogOut className="w-5 h-5 shrink-0" />
            <span
              className={`font-bold text-[12px] uppercase tracking-widest transition-all duration-300 ${
                (isOpen || isMobile) ? 'block ml-3' : 'hidden ml-0'
              }`}
            >
              LOGOUT
            </span>
